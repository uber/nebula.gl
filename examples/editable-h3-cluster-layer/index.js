...
function Button({ active, ...rest }) {
  return (
    <button style={{ backgroundColor: active ? "#50a0f0" : "" }} {...rest} />
  );
}
...
function Toolbar({
  ...
  return (
    <div style={{ position: "absolute", top: 0, right: 0, display: "grid" }}>
      ...
      <Button
        onClick={() => setMode(() => DrawRectangleMode)}
        active={mode === DrawRectangleMode}
      >
        Rectangle
      </Button>
      <Button
        onClick={() => setMode(() => DrawPointMode)}
        active={mode === DrawPointMode}
      >
        Point
      </Button>
      <div style={{ height: "7px" }} />
      <Button
        onClick={() => setBooleanOperation("union")}
